...
if run_analysis:
    ...
    if vol_df.empty:
        ...
        fig_box = px.box(vol_melt, x="종목", y="로그 수익률 (%)", color="종목",
                         title="월별 로그 수익률 분포", template="plotly_dark")
    ...
    if len(mean_returns) >= 2:
        heatmap_df = pd.DataFrame({'year': mean_returns.index.year, 'month': mean_returns.index.month, 'return': mean_returns.values})
        ...
        if not heatmap_data.empty and not heatmap_data.isnull().all().all():
            ...
            fig_heat = go.Figure(data=go.Heatmap(
                ...
                text=heatmap_data.values.round(2), texttemplate='%{text:.2f}'
            ))
            fig_heat.update_layout(title="월별 평균 로그수익률 (%)", xaxis_title="월", yaxis_title="연도", template="plotly_dark", height=500)
            st.plotly_chart(fig_heat, use_container_width=True)
        else:
            st.warning("⚠️ 히트맵 데이터가 모두 비어 있습니다.")
    else:
